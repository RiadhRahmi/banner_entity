...
/**
 * Provides a form for deleting a Banner revision.
 *
...
class BannerRevisionDeleteForm extends ConfirmFormBase {
  ...
  /**
   * Constructs a new BannerRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('banner'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Banner: deleted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    drupal_set_message(t('Revision from %revision-date of Banner %title has been deleted.', ['%revision-date' => format_date($this->revision->getRevisionCreationTime()), '%title' => $this->revision->label()]));
